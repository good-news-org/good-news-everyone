...
const sendMessageToTokens = (recipients: Recipients, payload) => {
  return admin
    .messaging()
    .sendToDevice(recipients.tokens, payload)
    .then(response => {
    ...
    return getGroupPost(groupId, postId)
      .then(post =>
        setPostCommentsCount(
          groupId,
          postId,
          increase(post.get('comments_count'))
        )
      )
...
export const onCreateFirestoreUser = functions.auth
  .user()
