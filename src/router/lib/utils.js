  ...
  for (let i = 0, l = ranked.length; i < l; i++) {
    ...
    for (; index < max; index++) {
      ...
      if (dynamicMatch && !isRootUri) {
        ...
        invariant(
          matchIsNotReserved,
          `<Router> dynamic segment "${dynamicMatch[1]}" is a reserved name. Please use a different name in path "${
            route.path
          }".`
        );
...
let rankRoutes = routes =>
  routes.map(rankRoute).sort((a, b) => (a.score < b.score ? 1 : a.score > b.score ? -1 : a.index - b.index));
